...
int main() {
    ...
    if (snrt_is_dm_core()) {
        // load_C(Batch, M, N, local_c, C, strideInnermostC, ldC, strideC);
    ...
    if (snrt_global_core_idx() == 0) {
        ...
        set_conv_block_gemm_csr(K, N, M, subtraction_setting, csr0, csr1, csr2,
                                M * N, bypassSIMD);
        ...
        if(!bypassSIMD){
            err += check_conv_result_D8(local_d8, D8_direct_conv2d, Batch, M, N);
        }else{
            err += check_conv_result_D32(local_d32, D32_direct_conv2d, Batch, M, N);
        }
